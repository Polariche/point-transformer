...
class GeoConv(nn.Module):
    def __init__(self, in_feat, out_feat, c, kernel=3, strides=1, padding=0, dilation=1, nonlin=None, train_c=False, mode='naive'):
        super().__init__()
        assert mode in ['naive', 'model1', 'model1_only_weight', 'model1_only_sort']
    ...
    def forward(self, x):
        ...
        if self.type in ['model1_only_weight', 'model1_only_sort', 'model1']:
            ...
            if self.type in ['model1_only_sort', 'model1']:
            ...
            if self.type in ['model1_only_weight', 'model1']:
